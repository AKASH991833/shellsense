...
def _suggest_single_word(
    ...
    for r in results:
        name = str(r.get("name", ""))
        r["_popularity"] = get_popularity_for(db, name)
        r["_history_freq"] = get_history_frequency_for(db, name)
        context_category = context.session_category if context else ""
        if context_category and r.get("category") == context_category:
            r["_category_bonus"] = True
...
def _predict_multi_word(
    ...
    if not cmd:
        return suggest_commands(db, command_part, limit=limit)
    ...
    if not predictions:
        return suggest_commands(db, query, limit=limit)

    return predictions[:limit]


def _predict_single_word(
    db: DatabaseManager,
    query: str,
    limit: int = 10,
) -> list[dict[str, object]]:
